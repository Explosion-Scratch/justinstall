...
const EXTENSION_CONFIG = {
  ...
  // Installer script types (platform-specific)
  installerScripts: {
    command: { platforms: ["darwin"], priority: 10, installable: true, executable: true },
    sh: { platforms: ["darwin", "linux", "freebsd", "openbsd"], priority: 8, installable: true, executable: true },
    bash: { platforms: ["darwin", "linux", "freebsd", "openbsd"], priority: 8, installable: true, executable: true },
    zsh: { platforms: ["darwin", "linux", "freebsd", "openbsd"], priority: 7, installable: true, executable: true },
    bat: { platforms: ["win32"], priority: 10, installable: true, executable: true },
    cmd: { platforms: ["win32"], priority: 9, installable: true, executable: true },
    ps1: { platforms: ["win32"], priority: 8, installable: true, executable: true },
  },

  // Platform-specific boosters (preferred formats)
  platformBoosters: {
    darwin: ["dmg", "pkg", "app", "command", "sh"],
    linux: ["AppImage", "deb", "rpm", "sh", "bash"],
    win32: ["exe", "msi", "bat", "cmd", "ps1"],
  },

  // All installable extensions (union of archives, packages, and installer scripts)
  get installable() {
    return [
      ...Object.keys(this.archives),
      ...Object.keys(this.packages),
      ...Object.keys(this.installerScripts)
    ];
  },
};
...
/**
 * Check if an extension is an installer script type
 */
const isInstallerScriptExtension = (extension) => {
  if (!extension) return false;
  return Object.keys(EXTENSION_CONFIG.installerScripts).includes(extension.toLowerCase());
};

/**
 * Get installer script config for an extension
 */
const getInstallerScriptExtensionConfig = (extension) => {
  if (!extension) return null;
  return EXTENSION_CONFIG.installerScripts[extension.toLowerCase()] || null;
};
...
const getExtensionInfo = (extension) => {
  ...
  if (isInstallerScriptExtension(extension)) {
    return { type: "installer_script", ...EXTENSION_CONFIG.installerScripts[extension.toLowerCase()] };
  }
  return null;
};
...
module.exports = {
  ...
  getInstallerScriptExtensionConfig,
