...
const selectBestAsset = (assets, platformInfo, capabilities) => {
  ...
  let compatible = assets
    .map((asset) => {
      ...
      // Cross-platform extension penalty (e.g., .exe on macOS)
      if (asset.extension === "exe" && platform !== "win32") {
        asset.points -= 2;
      }
      if (asset.extension === "msi" && platform !== "win32") {
        asset.points -= 2;
      }
  ...
  const platformSpecificAssets = compatible.filter(asset =>
    asset.compatiblePlatforms.length > 0
  );

  if (platformSpecificAssets.length > 0) {
    // Among platform-specific assets, prioritize installable extensions
    const installableExtensions = ["pkg", "dmg", "app", "deb", "rpm", "tar.gz", "zip", "tar.zst", "AppImage", "exe", "msi", "tar.bz2", "tar.xz", "7z"];
    const installablePlatformAssets = platformSpecificAssets.filter(asset =>
      installableExtensions.includes(asset.extension)
    );

    if (installablePlatformAssets.length > 0) {
      return installablePlatformAssets[0];
    }
    
    // Return the highest scoring platform-specific asset
    return platformSpecificAssets[0];
  }

  // Fallback to assets with installable extensions (for websites without platform indicators)
  const installableExtensions = ["pkg", "dmg", "app", "deb", "rpm", "tar.gz", "zip", "tar.zst", "AppImage", "exe", "msi", "tar.bz2", "tar.xz", "7z"];
...
const getBinaries = (dir) => {
  ...
  binaries.push(
    ...allFiles.filter((i) =>
      ALLOWED_EXTENSIONS.find((j) => i.toLowerCase().endsWith(j)) && !isIgnored(i)
    )
  );
  ...
  const executableFiles = allFiles.filter(f => isExecutable(f) && looksLikeBinary(f) && !isIgnored(f));
  const potentialBinaries = allFiles.filter(f => looksLikeBinary(f) && !executableFiles.includes(f) && !isIgnored(f));
...
const installBinaries = async (
  ...
  for (const binary of binaries) {
    ...
    const cleanName = extractName({ name: path.basename(binary) }) || path.basename(binary);
