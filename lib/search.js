...
/**
 * Repository selection interface for direct search results
 */
const selectRepositoryFromResults = async (repos, originalQuery) => {
  const log = createLogger();
  const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
  ...
  return new Promise((resolve) => {
    const selectFromCurrentResults = () => {
      rl.question(
          ...
          if (
            ...
            rl.question("Enter search query: ", async (query) => {
              try {
                ...
                selectRepository(newRepos.slice(0, 10), rl, resolve, () => {});
              } catch (error) {
                log.error(`Search failed: ${error.message}`);
                selectFromCurrentResults();
              }
...
/**
 * Display search results directly without interactive selection
 */
const displaySearchResults = async (query, options = {}) => {
  const log = createLogger();
  ...
  if (repos.length === 0) {
    log.warn(`No repositories found matching "${query}"`);
    return null;
  }

  log.log(`\nFound ${repos.length} repository(ies) for "${query}":`);
  repos.forEach((repo, index) => {
    log.log(`${index + 1}. ${repo.full_name} (⭐ ${repo.stargazers_count})`);
    log.log(`   ${repo.description || "No description"}`);
    log.log("");
  });
