...
const parseSource = (input) => {
  ...
  if (isURL) {
    if (input.includes("github.com")) {
      ...
      const releaseMatch = input.match(
        /github\.com\/([^\/]+)\/([^\/]+)\/releases\/tag\/([^\/]+)/i,
      );
      if (releaseMatch) {
...
const getGitHubAssets = async (owner, repo, specificTag = null) => {
  ...
  if (specificTag) {
    ...
    if (response.ok) {
      ...
      throw new Error(
        `Failed to fetch GitHub releases: ${response.statusText}`,
      );
    }
  }
...
const processAssetFromLink = (href, baseUrl, linkText) => {
  try {
    ...
    // Check if URL path contains "installers" - common pattern for software sites
    const isInstallerPath = url.pathname.toLowerCase().includes("/installers/");

    // Check if the link text suggests this is an installer/download
    const cleanLinkText = linkText
      .replace(/<[^>]*>/g, "")
      .trim()
      .toLowerCase();
    const isInstallerLinkText =
      cleanLinkText.includes("installer") ||
      cleanLinkText.includes("download") ||
      /\d+-bit/.test(cleanLinkText) ||
      cleanLinkText.includes("x64") ||
      cleanLinkText.includes("x86") ||
      cleanLinkText.includes("arm") ||
      cleanLinkText.includes("intel");

    if (!hasInstallableExtension && !couldBeExecutable && !isInstallerPath) {
      return null;
    }
...
const SCRIPT_DETECTION_CONFIG = {
  ...
  REJECTION_PATTERNS: [
    /\s*\|.*\|/, // Table/pipe formatting
    /\s*`[^`]+`/, // Backtick formatting
...
const getScriptPlatformCompatibility = (code) => {
  ...
  const lowerCode = code.toLowerCase();
  const patterns =
    PLATFORM_SCRIPT_PATTERNS[platform] || PLATFORM_SCRIPT_PATTERNS.darwin;
